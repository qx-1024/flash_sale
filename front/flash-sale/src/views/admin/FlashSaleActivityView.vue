<template>
    <el-button class="mybtn" :icon="Plus" @click="add" size="small" round>添 加 闪 购</el-button>
    <img class="empty_data_img" src="../../assets/empty_data.svg" alt="空空如也~" v-if="tableData.length == 0">
    <el-table :data="tableData" stripe style="width: 100%" v-else>
        <el-table-column prop="activityName" align="center" label="闪购活动名称" show-overflow-tooltip/>
        <el-table-column prop="productName" align="center" label="商品名称" show-overflow-tooltip/>
        <el-table-column prop="startTime" align="center" label="开始时间" show-overflow-tooltip/>
        <el-table-column prop="endTime" align="center" label="结束时间" show-overflow-tooltip/>
        <el-table-column prop="activityStatus" align="center" label="活动状态"/>
        <el-table-column prop="createTime" align="center" label="添加时间" show-overflow-tooltip/>
        <el-table-column prop="options" align="center" fixed="right" label="操作" width="180">
            <template #default="scoped">
                <el-button type="success" :icon="Edit" size="small" @click="view(scoped.row.activityId)" plain round>编 辑</el-button>
                <el-button type="danger" :icon="Delete" size="small" @click="del(scoped.row.activityId)" plain round>删 除</el-button>
            </template>
        </el-table-column>
    </el-table>


    <!-- 分页组件 -->
    <el-pagination
        class="mt-4"
        layout="prev, pager, next"
        :page-size="size"
        :total="total"
        @prev-click="toPage"
        @next-click="toPage"
        @current-change="toPage"
    />


    <!-- 详情与编辑对话框 -->
    <el-dialog v-model="viewAndEditDialogVisible" title="编辑闪购活动" width="500" center>
        
        <template #footer>
        <div class="dialog-footer">
            <el-button @click="viewAndEditDialogVisible = false">取 消</el-button>
            <el-button type="primary" @click="commitEdit">确 定</el-button>
        </div>
        </template>
    </el-dialog>

    <!-- 添加对话框 -->
    <el-dialog v-model="addDialogVisible" title="新增闪购活动" width="500" center>
        <el-form :model="addForm" label-width="auto" style="max-width: 600px">
            <el-form-item label="活动名称">
                <el-input v-model="addForm.activityName" />
            </el-form-item>
            <el-form-item label="商品名称">
                <el-input v-model="addForm.productName" />
            </el-form-item>
            <el-form-item label="活动时间">
                <el-date-picker
                    v-model="addForm.dateRange"
                    type="daterange"
                    range-separator="-"
                    start-placeholder="开始时间"
                    end-placeholder="结束时间"
                    value-format="YYYY-MM-DD HH:mm:ss"
                />
            </el-form-item>
        </el-form>


        <template #footer>
        <div class="dialog-footer">
            <el-button @click="addDialogVisible = false">取 消</el-button>
            <el-button type="primary" @click="commitAdd">确 定</el-button>
        </div>
        </template>
    </el-dialog>
</template>


<script setup>
import { ref, onMounted } from "vue";
import { Plus, Delete, Edit, View } from '@element-plus/icons-vue'
import { doGet } from "../../http/httpRequest";

const tableData = ref([])

onMounted(() => {
    loadData(1)
})

/****************************************** 分页 *********************************************/
const num = ref(0)
const size = ref(0)
const total = ref(0)

/**
 * @description 页面跳转
 */
const toPage = (current) => {
    loadData(current)
}

/**
 * @description 加载闪购活动信息
 */
const loadData = (current) => {
    doGet('/activity/page',{
        current: current
    }).then(res => {
        if(res.data.code === 200){
            tableData.value = res.data.data.records
            num.value = res.data.data.current
            size.value = res.data.data.size
            total.value = res.data.data.total

            tableData.value.forEach(item => {
                switch (item.activityStatus) {
                    case 0:
                        item.activityStatus = '未开始'
                        break;
                    case 1:
                        item.activityStatus = '已结束'
                        break;
                    case 2:
                        item.activityStatus = '进行中'
                        break;
                }
            })
        }
    })
}


/*************************************** 详情与编辑 ******************************************/
const viewAndEditDialogVisible = ref(false)

const addForm = ref({})

const view = () => {
    viewAndEditDialogVisible.value = true
}

const commitEdit = () => {
    viewAndEditDialogVisible.value = false
}

/*************************************** 添加 ******************************************/
const addDialogVisible = ref(false)

const add = () => {
    addDialogVisible.value = true
}

const commitAdd = () => {
    addDialogVisible.value = false
}


</script>


<style scoped>
.el-table {
    margin-top: 20px;
    margin-bottom: 20px;
}

.empty_data_img {
    width: 200px;
    position: absolute;
    left: 50%;
    top: 50%;
    transform: translateX(-25%) translateY(-50%);
}

.mybtn {
    border: none;
    color: var(--flash-lighterblue-lighter-4);
    background-color: var(--flash-skyblue-color);
}

.mybtn:hover {
    background-color: var(--flash-skyblue-lighter-1);
}

.el-pagination {
    position: absolute;
    bottom: 35px;
    left: 50%;
    transform: translateX(-30%);
}
</style>