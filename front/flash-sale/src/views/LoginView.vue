  ...
  <div class="container">
    ...
    <el-card class="loginCard" shadow="hover" v-show="isLogin">
        ...
        >
            ...
            <el-form-item label="密  码" prop="password">
                <el-input v-model="loginQuery.password" type="password" placeholder="请输入密码"/>
            </el-form-item>

             <el-form-item label="验证码" prop="code">
                <el-input style="width: 180px; margin-right: 5px" v-model="loginQuery.code" placeholder="请填写验证码" />
                <Vertitatioin />
            </el-form-item>
            
            <el-form-item class="options">
                <el-space wrap :size="310">
                    <el-link type="error" :underline="false" @click="forget">忘记密码？</el-link>
    ...
    <el-card class="registerCard" shadow="hover" v-show="isRegister">
        ...
        >
            ...
            <el-form-item label="密 码" prop="password">
                <el-input v-model="registryQuery.password" type="password" placeholder="请输入密码"/>
            </el-form-item>
            ...
            <el-form-item label="手 机" prop="phoneNumber">
                <el-input v-model="registryQuery.phoneNumber" placeholder="请输入手机号码"/>
            </el-form-item>
    ...
    <el-card class="forgetPasswordCard" shadow="hover" v-show="forgetPassword">
        ...
        >
            ...
            <el-form-item label="账 号">
                <el-input v-model="forgetPasswordQuery.account" placeholder="请输入账号"/>
            </el-form-item>

            <el-form-item label="新 密 码">
                <el-input v-model="forgetPasswordQuery.newPassword" type="password" placeholder="请输入新密码"/>
            </el-form-item>

            <el-form-item label="确 认 密 码">
                <el-input v-model="forgetPasswordQuery.confirmPassword" type="password" placeholder="请输入确认密码"/>
            </el-form-item>
...
const register = () => {
    ...
    registryForm.value.validate(isValid => {
        if(isValid) {
            ...
            // 密码加密
            sha_256_encrypt(registryQuery.value.password).then(encryptedPassword => {
                registryQuery.value.password = encryptedPassword;

                let json = JSON.stringify(registryQuery.value);

                doPost("http://localhost:8030/user/save", json).then(resp => {
                    if(resp.data.code === 200){
                        ElMessage({
                            message: '注册成功',
                            type: 'success',
...
const confirmCommit = () =>{
    ...
    forgetPasswordForm.value.validate(isValid => {
        if(isValid){
            sha_256_encrypt(forgetPasswordQuery.value.newPassword).then(newEncryptPassword => {
                sha_256_encrypt(forgetPasswordQuery.value.confirmPassword).then(confirmEncryptPassword => {
                    if(newEncryptPassword === confirmEncryptPassword){
                        doPut("http://localhost:8030/user/updatePassWord", {
